...
function M:on_deposit(amount:int)
    ...
    local deposit_info:string = tostring(caller_address).." transfered in,amount "..tostring(amount)..",sum "..tostring(get_contract_balance_amount(get_current_contract_address(), "ALP")+amount)
...
function M:handle(address:string)
    ...
    for k,v in pairs(self.storage.participant) do  --遍历参与者列表，只对在参与者列表中的地址进行理赔
        if address == v then
            ...
            if amount > 10000000 then   --最高赔付100ALP
                amount = 10000000
            end
            local res=transfer_from_contract_to_address(address,"ALP",amount)
